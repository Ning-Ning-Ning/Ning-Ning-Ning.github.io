...
GitHub Pages 页面通过 Supabase Data API 读取 `public.daily_curves`，按日期展示实际、预测、调整后用电量（图标题为“用户用电量曲线”），以及日前电价和实时电价。
...
- 用电量图（标题“用户用电量曲线”）位于页面顶部，其日期选择器在图表标题右侧；页面下方为日前电价与实时电价双折线图，两条曲线共用同一坐标系，单位为元/MWh；**该图使用独立的日期选择器**（与用电量图的日期互不影响），悬停可查看每个时段价格。
- 24 时段详细数据与电价图之间提供“电费变化”热力表：`电费变化 = (日前价格 - 实时价格) × (调整后用电量 - 预测用电量)`。仅使用当前登录账号在 `user_adjustments` 中实际保存的调整值；没有调整记录或缺少必要价格/预测数据的时段变化为 0。筛选控件仅选择月份，默认当前月份并一次展示整月数据；**表格第一行为“全月汇总”**，每列（时段）显示该时段整月总计，最右列显示整月合计（不再在表上方单独展示）；色块模式下汇总行的时段总计同样不显示数字。每行一天、每列一个小时，最右列显示当日电费变化。正数（电费增加）使用蓝色，负数（电费减少）使用红色；支持纯色块/数字模式切换，所有日期行完整展开。
