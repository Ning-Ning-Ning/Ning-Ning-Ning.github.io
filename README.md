...
- 24 时段详细数据与电价图之间提供“策略收益”热力表：`收益 = (日前价格 - 实时价格) × (调整后用电量 - 预测用电量)`。仅使用当前登录账号在 `user_adjustments` 中实际保存的调整值；没有调整记录或缺少必要价格/预测数据的时段收益为 0。每行一天、每列一个小时，最右列计算并显示当日总收益；支持日期范围、纯色块/数字模式切换，所有日期行完整展开。
